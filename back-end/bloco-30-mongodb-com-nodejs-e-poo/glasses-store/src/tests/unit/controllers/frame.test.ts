...
describe('Frame Controller', () => {
  ...
  before(() => {
    ...
    sinon.stub(frameService, 'read').resolves(frameMockWithIdArray);
    sinon.stub(frameService, 'update').resolves(frameMockWithId);
    sinon.stub(frameService, 'remove').resolves(frameMockWithId);
  ...
  describe('Read Frames', () => {
    it('Success', async () => {
      ...
      expect((res.status as sinon.SinonStub).calledWith(200)).to.be.true;
      expect((res.json as sinon.SinonStub).calledWith(frameMockWithIdArray)).to.be.true;
    });
  });
  ...
  describe('Remove Frames', () => {
    it('Success', async () => {
      req.params = { id: frameMockWithId._id };
      await frameController.remove(req, res);
